...
export default function DownloadButton({ large = false, fancy = false }: { large?: boolean; fancy?: boolean }) {
  return (
    <a
      ...
      className={`group relative inline-flex items-center justify-center gap-2.5 rounded-xl bg-[--color-coral] hover:bg-[--color-coral-600] font-semibold text-white overflow-hidden transition-transform hover:scale-[1.02] active:scale-95 ${fancy ? "btn-download shadow-[0_16px_40px_-14px_rgba(240,118,74,0.7)]" : ""} ${large ? "px-6 py-3.5 text-[15px]" : "px-5 py-2.5 text-sm"}`}
    >
      <svg width={large ? 18 : 16} height={large ? 18 : 16} viewBox="0 0 24 24" fill="currentColor" aria-hidden>
